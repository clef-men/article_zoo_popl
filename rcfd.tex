...
It is used in \Eio in two different ownership regimes: (1) any thread can try to access or close the FD; (2) any thread can try to access the FD but only the owner thread can close it --- and is responsible for closing it.
...
The $\mathit{open}$ function is given a fraction of $\iPredTwo$ to access the FD.
...
It yields $\closing\ t$, a persistent witness that $t$ has been flagged as closed.
The wrapped FD is not closed immediately.
It will be closed only once possible, meaning all ongoing \ocamlinline{use} calls owning a fraction of the FD end.
