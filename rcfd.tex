...
\ocamlinline{Rcfd} provides a parallelism-safe file descriptor (FD) relying internally on reference-counting.
...
To control the postconditions of the specification and the weakest preconditions, the user can choose an arbitrary predicate $\iPredThree$ parameterized by a boolean indicating whether the $\mathit{closed}$ (\ocamlinline{false}) or the $\mathit{open}$ (\ocamlinline{true}) was called.
...
The `closing' thread is the one that succeeds in updating the \ocamlinline{Closing} constructor (to a new one carrying a no-op function) using again \ocamlinline{Atomic.Loc.compare_and_set}.
...
In fact, the \ocamlinline{Closing} constructor also has to be generative to prevent \emph{sharing}: when the `closing' thread wins the \ocamlinline{Atomic.Loc.compare_and_set}, it must be true that the `closing' function has not changed.
