...
\begin{enumerate}
  \item
    Our first implementation\footnote{\urlAnonymous{https://github.com/ocaml/ocaml/pull/13404}} introduces a built-in type \mintinline{ocaml}{'a Atomic.Loc.t} for an atomic location that holds an element of type \mintinline{ocaml}{'a}, with a syntax extension \mintinline{ocaml}{[%atomic.loc <expr>.<field>]} to construct such locations.
    Atomic primitives operate on values of type \mintinline{ocaml}{'a Atomic.Loc.t} and they are exposed as functions of the module \mintinline{ocaml}{Atomic.Loc}.
...
\end{minted}
  ...
  \item
    Our second implementation\footnote{\urlAnonymous{https://github.com/ocaml/ocaml/pull/13707}} introduces a built-in type \mintinline{ocaml}{('r, 'a) Atomic.Field.t} that denotes a field/index of type \mintinline{ocaml}{'a} within a record of type \mintinline{ocaml}{'r}, with a syntax extension \mintinline{ocaml}{[%atomic.loc <field>]} to construct such field description, and atomic primitives in a module \mintinline{ocaml}{Atomic.Field}, that need both the record value of type \mintinline{ocaml}{'r} and the field description.
    For example, the standard library exposes:
\begin{minted}{ocaml}
...
\end{minted}
    where \mintinline{ocaml}{[%atomic.field front]} has type \mintinline{ocaml}{('a bag, int) Atomic.Loc.t}.
    Internally, a value of type \mintinline{ocaml}{('r, 'a) Atomic.Field.t} is just an integer offset locating the field within the record: in exchange for a more complex type, we get a simpler data representation, that does not rely on specific compiler optimizations to generate efficient code, even across abstraction boundaries.
    Note that the previous type \mintinline{ocaml}{'a Atomic.Loc.t} can be reconstructed as a dependent pair of a \mintinline{ocaml}{'r} and a \mintinline{ocaml}{('r, 'a) Atomic.Field.t}, which is expressible in \OCaml as a GADT:
\begin{minted}{ocaml}
...
\end{minted}
    ...
    The extension form
    \mintinline{ocaml}{[%atomic.loc buf.front]} has typing rules that are very similar to a field access \mintinline{ocaml}{buf.front}.
    On the other hand,
    \mintinline{ocaml}{[%atomic.loc front]} interacts in a non-trivial way with the \OCaml machinery for type-based disambiguation of record fields---several records with a field named \mintinline{ocaml}{front} can co-exist in the typing environment.
