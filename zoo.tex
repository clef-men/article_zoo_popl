...
The core of \Zoo is \ZooLang~\refTheories{zoo/language/syntax.v}: a concurrent, imperative, untyped, functional programming language fully formalized in \Rocq.
Its semantics has been designed to match \OCaml.

\ZooLang comes with an \Iris-based program logic~\refTheories{zoo/program_logic/wp.v}, proved correct with respect to its small-step operational semantics~\refTheories{zoo/language/semantics.v}.
The reasoning rules are expressed in separation logic (including rules for the different constructs of the language) along with \Rocq tactics~\refTheories{zoo/proofmode.v} that integrate into the \Iris proof mode~\citep{DBLP:journals/pacmpl/KrebbersJ0TKTCD18}.
...
The \ZooLang syntax~\refTheories{zoo/language/notations.v} is given in \cref{fig:zoo}%
\footnote{
