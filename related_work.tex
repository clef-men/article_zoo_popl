...
(2) \Osiris is based on \Iris and concentrates on a very expressive semantics with advanced, difficult language features to formalize -- exceptions and, in particular, effect handlers following \citet*{DBLP:journals/pacmpl/VilhenaP21}. On the other hand, it does not support concurrency, and its perfectionist rather than pragmatic treatment of issues such as undetermined evaluation order make it harder to use than \HeapLang or \Zoo for program verification; it may not yet be ready for practical verification at scale -- certainly not for our problem domain.

(3) \DRFCaml is based on \Iris and does support concurrency.
It is mostly an extension of \HeapLang with features (modalities and stack regions) entirely orthogonal to our work. It distinguishes atomic and non-atomic locations, with a sequential memory model for atomics and undefined behavior for non-atomic races. This implies that programs whose safety can be verified in the program logic are data-race free, and thus their strongly-sequential semantics is compatible with  \OCamlFive thanks to the DRF property~\citep*{DBLP:conf/pldi/DolanSM18}. In other words, DRFCaml has a sequentially-consistent memory model, with a static discipline enforcing that programs that would allow different behaviors in \OCamlFive are rejected. But the concurrent programs that we want to verify do commonly rely on non-atomic races for performance reason (or could use relaxed atomics if those were available), so they cannot be expressed in \DRFCaml.
