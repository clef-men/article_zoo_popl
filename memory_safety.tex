...
we proposed adding (sequential) resizable arrays
\footnote{
\url{https://github.com/ocaml/ocaml/pull/11882}
}
to the \OCaml standard library.
...
% \item
  Memory-safety relies on a \emph{weak} invariant that does not
...
\ocamlinline{push} function is the following:
\[
  \iSpec{
...
The definition of the semantic type for dynarrays, and the statement of memory safety (or in fact semantic typing) for \ocamlinline{push} look as follows:
...
Similar problems of memory-safety occur in the concurrent data structures of the \Saturn library. For example, we explained (\cref{par:michael-scott-cleanup}) that the \Saturn implementation of the Michael-Scott MPMC queue is careful to ``erase'' values stored in the queue to avoid memory leaks. This erasure is performed by writing a non-type-safe dummy value, \ocamlinline{Obj.magic ()}. We define a semantic type~\refTheories{zoo_saturn/mpmc_queue_1.v\#L962} for the MPMC queue which is essentially its concurrent invariant, and prove that this unsafe idiom does not endanger memory-safety.
