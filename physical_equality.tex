...
Given such guarantees, denoted by \mintinline{coq}{val_physeq} and \mintinline{coq}{val_physneq}, the non-deterministic semantics is reflected in the logic through the following specification:
...
Hence, if physical equality on two generative blocks returns \mintinline{ocaml}{true}, these blocks are definitionally equal in \Rocq.
...
Most formalizations of physical equality in the literature do not give any guarantee when physical equality returns \mintinline{ocaml}{false}.
Many use-cases of physical equality, in particular retry loops, can be verified with only sufficient conditions on \mintinline{ocaml}{true}.
...
As there is only one \mintinline{ocaml}{Open} block whose identifier does not change, it cannot be the case that the current state is \mintinline{ocaml}{Open}, hence it is \mintinline{ocaml}{Closing}.
