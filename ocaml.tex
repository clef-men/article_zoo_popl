...
In May-June 2024 we proposed a design for atomic record fields as an \OCaml language change proposal: RFC~\#39\footnote{\urlDeanonymizing{https://github.com/ocaml/RFCs/pull/39}}.
...
\begin{ocamlcode}
val Atomic.Field.fetch_and_add :
  'a -> ('a, int) Atomic.Field.t -> int -> int
\end{ocamlcode}
    and users can write:
\begin{ocamlcode}
...
\end{ocamlcode}
  ...
  \item
    ...
    For example, the standard library exposes:
\begin{ocamlcode}
val Atomic.Loc.fetch_and_add :
  int Atomic.Loc.t -> int -> int
\end{ocamlcode}
    and users can write:
\begin{ocamlcode}
let preincrement_front (buf : 'a bag) : int =
...
%
interacts in a non-trivial way with the \OCaml machinery for type-based disambiguation of record fields---several records with a field named \ocamlinline{front} can co-exist in the typing environment.
...
In April and May, we managed to build consensus among \OCaml maintainers that the simple design, with a simpler user-facing design and a simpler implementation, was a good compromise in view of the significant difficulties of the full design with inline records. Our implementation of the simple design was reviewed again, we wrote user documentation, and it was integrated in OCaml 5.4, released on August 2024.
